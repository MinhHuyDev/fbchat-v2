...
def shell():
    ...
    try:
        ...
        return 'Cannot retrieve speedtest configuration'
    ...
    if not args.mini:
        ...
        try:
            speedtest.get_servers(servers=args.server, exclude=args.exclude)
                ...
                return 'No matched servers: %s' %', '.join('%s' % s for s in args.server)
          
        except (ServersRetrievalError,) + HTTP_ERRORS:
            return 'Cannot retrieve speedtest server list'
            raise SpeedtestCLIError(get_exception())
    ...
    if args.download:
        ...
        download = "Skipping download test"
    ...
    if args.upload:
        ...
        upload = "Skipping upload test"



    if args.share and not machine_format:
        pass
    jsonSpeedtest = json.loads(str(results.json()))
    sponsor = str(jsonSpeedtest["server"]["sponsor"]) #nhà tài trợ
    namecity = str(jsonSpeedtest["server"]["name"])
    testing = str(jsonSpeedtest["client"]["isp"])
    ipaddress = str(jsonSpeedtest["client"]["ip"])
    return """📉Download: """+str(download)[:6]+""" m"""+str(args.units[0])+"""/s\n📈Upload: """+str(upload)[:6]+""" m"""+str(args.units[0])+"""/s\n🛸Ping: """+str(results.ping)+""" ms\n👨‍💻Sponsor: """+sponsor+""" ("""+namecity+""")\n🏢Testing from: """+testing+""" ("""+ipaddress+""")\n🖼️Images_Results: """+ results.share() + """"""
...
def main():
    try:
        ...
        if getattr(e, 'code', 1) not in (0, 2):
            return 'Đã xảy ra lỗi: %s' % e
            if not msg:
                return 'Đã xảy ra lỗi: %r' % e
