...
def _write_distributions(dist_dir: Path, checksum_source: str) -> None:
    dist_dir.mkdir()
    with zipfile.ZipFile(dist_dir / "fbchat_v2-2.2.2-py3-none-any.whl", "w") as wheel:
        wheel.writestr(CHECKSUM_MODULE_MEMBER, checksum_source)

    payload = checksum_source.encode()
    with tarfile.open(dist_dir / "fbchat_v2-2.2.2.tar.gz", "w:gz") as sdist:
        info = tarfile.TarInfo(f"fbchat_v2-2.2.2/src/{CHECKSUM_MODULE_MEMBER}")
...
def test_bind_and_verify_final_distributions(tmp_path: Path) -> None:
    ...
    assert bind_checksums(binaries, module, manifest, version="2.2.2") == expected
    ...
    wheel, sdist = verify_distributions(
        binaries, dist, version="2.2.2", manifest_path=manifest
    )
...
def test_verify_rejects_checksum_drift(tmp_path: Path) -> None:
    ...
    module = tmp_path / "source" / "_bridge_checksums.py"
    manifest = binaries / "SHA256SUMS"
    bind_checksums(binaries, module, manifest, version="2.2.2")
    stale = dict(digests)
    stale[sorted(stale)[0]] = "0" * 64
    dist = tmp_path / "dist"
    _write_distributions(dist, render_checksum_module("2.2.2", stale))

    with pytest.raises(RuntimeError, match="checksum không khớp"):
        verify_distributions(binaries, dist, version="2.2.2", manifest_path=manifest)
...
def test_validate_release_tag_requires_exact_project_version() -> None:
    validate_release_tag("v2.2.2", "2.2.2")
    with pytest.raises(RuntimeError, match="không khớp"):
        validate_release_tag("v2.2.1", "2.2.2")
